      ...
      <el-table-column label="操作" width="230" align="center">
        <template slot-scope="scope">
          <el-button
            ...
            type="primary"
            size="mini"
            @click="update(scope.row.id, 0)">下线
          </el-button>
          <el-button
...
<script>
import { getHospList, findByDictCode, findChildId, updateStatus } from '@/api/hosp'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    update(id, status) {
      updateStatus(id, status)
        .then(response => {
        // 刷新页面
          this.fetchData(this.page) // 刷新当前页或者第一页
        })
    },
    // 查询医院列表
    fetchData(page = 1) {
    ...
    // 查询所有省份
    findAllProvince() {
      findByDictCode('Province') // Province就是dictCode 固定的
        .then(response => {
          this.provinceList = response.data
        })
    },
    // 点击省 显示 市 （联动）点击省就调用查询市的方法
    provinceChanged() {
      // 初始值
      this.cityList = []
      this.searchObj.cityCode = null
      // 调用方法，根据省id,查询下面的子节点（市）
      findChildId(this.searchObj.provinceCode) // 双向绑定，由输入框输入
        .then(response => {

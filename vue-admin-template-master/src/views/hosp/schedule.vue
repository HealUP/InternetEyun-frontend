...
<script>
import { getSchedulByHoscode, getScheduleRules, getScheduleDetail } from '@/api/hosp'
export default {
  data() {
    return {
      listLoading: true, // 加载图标
      data: [],
      defaultProps: {
  ...
  created() { // 页面渲染之前执行 获取数据
    ...
    this.workDate = this.getCurDate() // 默认选中第一个
    this.fetchData()
  },
  methods: {
    // 查询排班详情
    getDetailSchedule() {
      getScheduleDetail(this.hoscode, this.depcode, this.workDate) // 默认的depcode,workDate 路由传递的hoscode
        .then(response => {
    ...
    getScheduleRule() { // 获取预约规则
      getScheduleRules(this.page, this.limit, this.hoscode, this.depcode).then(response => {
        ...
        this.baseMap = response.data.baseMap // 包含医院名字

        // 分页后workDate=null，默认选中第一个
        if (this.workDate == null) {
          this.workDate = this.bookingScheduleList[0].workDate // 默认选中的日期
        }
        // 调用查询排班详情
        this.getDetailSchedule() // 使用默认的workDate查询排班详情
        // 查询完毕使得加载图标不显示

...
<script>
import { showHospDetail } from '@/api/hosp'
export default {
  data() {
    return {
      hospital: null, // 医院信息
  ...
  created() {
    // 获取路由id
    const id = this.$route.params.id // 从隐藏路由中获取到改行记录的id值
    // 调用方法，根据id查询医院详情
  ...
  methods: {
    ...
    fetachHospDetail(id) {
      showHospDetail(id)
        .then(response => {
          this.hospital = response.data.hospital // 医院基本信息 map集合
          this.bookingRule = response.data.bookingRule // 预定信息
        })
    },
    // 返回医院列表
    back() {
